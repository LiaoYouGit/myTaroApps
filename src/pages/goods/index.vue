...
import { formatAmount, $navigateTo } from "@/lib/util";
...
function createSalesOrder(){
  ...
  Taro.setStorageSync('createSales', selectionList);
  $navigateTo('/pages/goods/createSales')
}
...
async function joinCart(){
  Taro.showLoading({title:'加载中'})
  const query = {
    skuIdList: selection.value.map(item=>({ skuId: item, num: 1 }))
  }
  await add(query)
  Taro.hideLoading()
  Taro.showToast({
...
<template>
  <view>
    ...
    >
      <nut-checkbox-group v-model="selection">
    ...
    <!-- 生成采购单或加入购物车 -->
    <view v-if="selection.length > 0" class="footer-nav">
      <nut-button class="left-btn" type="primary" size="small" @click="createSalesOrder">生成采购单</nut-button>
      <nut-button class="right-btn" type="primary" size="small" @click="joinCart">加入进货车</nut-button>
    </view>
  </view>
</template>
